...
class Tokenizer:
    ...
    def next(self):
        ...
        while True:
            if len(self._line) == 0:
                ...
                zatvkom = self._line.find("*/")
                i = self._line.find("//")
                if i != -1 and zatvkom == -1: 
                    self._line = self._line[:i]
                if i !=- 1 and zatvkom != -1:
                    if self._comment:
                        self._line = self._line[(zatvkom + 2):]
                        self._comment = False
    ...
    def _parseInt(self):
        ...
        while len(self._line):
            c = self._line[0]
            if c not in numberChars:
                self._error("Error, broj nije int")
                break
            else:
                ret = ret * 10 + int(c)
                self._line = self._line[1:]
        if ret > -32768 and ret < 32768: #ili ret > 0 -> ako je strogo prirodni onda ni -100 nebi smjelo biti okej...
            return ret
        else:
            self._error("Error, broj nije int")
